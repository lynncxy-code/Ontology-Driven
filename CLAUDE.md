...
| `d:\tmp\digital_twin_aircraft\ue_project\` | UE 5 工程（新建，M2 阶段）|
| `d:\tmp\digital_twin_aircraft\ue5\` | 现有 UE 数据孪生同步组件（不碰）|
...
### 冲突 A：Flask 静态文件路由 vs `/scenes` 路由

`app.py` 把整个 `frontend/` 挂为静态目录（`static_folder=frontend_dir, static_url_path=''`）。
这意味着 `frontend/scenes/scenes.html` 会被自动暴露为 `/scenes/scenes.html`，
而我们同时要在 `register_lite_routes` 里注册 `/scenes` 路由。

**规避方式**：在 `register_lite_routes` 里注册的路由是 `/scenes`（无尾缀），
Flask 显式路由优先级高于静态文件，不会冲突。
但如果出现 404 或文件加载异常，第一时间检查这里。

### 冲突 B：`/api/v2/assets` vs `/api/v3/lite/assets` 同名概念

现有 `app.py` 已有 `/api/v2/assets`（Nexus 的资产绑定逻辑，和本体/实例绑定有关）。
新增的 `lite_assets` 是**完全独立**的资产表（存 FBX 路径、USD 缓存等），两套系统没有关联。

**规避方式**：前端 `scenes.js` 只调用 `/api/v3/lite/assets`，绝不混用 `/api/v2/assets`。
后端 `lite/` 目录下的代码也不要 import 或调用现有的 `MOCK_ASSETS` / `v2/assets` 相关逻辑。
