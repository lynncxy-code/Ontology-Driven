"""Apply reviewed semantic names to round-two ZHHZ furniture types/instances."""
...
def apply(plan_path: str, dry_run: bool = False) -> dict:
    ...
    if len(type_names) != 10 or len(instance_names) != 16:
        raise ValueError("Furniture rename plan must contain exactly 10 types and 16 instances")

    store = ProjectStore()
    if store.get_active_id() != "ds_1784694647848":
    ...
    if missing_types or missing_instances:
        raise RuntimeError(
            f"Rename targets missing: types={missing_types}, instances={missing_instances}"
        )
    for instance_id in instance_names:
        rid = instances[instance_id].get("object_type_rid")
        if rid not in type_names:
            raise RuntimeError(f"Furniture instance {instance_id} has unexpected type {rid}")
    ...
    if updated_graph_nodes != len(type_names):
        raise RuntimeError(
            f"Expected to rename {len(type_names)} graph nodes, found {updated_graph_nodes}"
        )
