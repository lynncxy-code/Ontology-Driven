...
- 未分区实例保持显示。
- 不做资源卸载。
- 完成显隐切换后，对该 Zone 及其后代中的可见成员计算联合包围盒，并执行一次性平滑取景；未分区常驻实例不参与取景计算，0 匹配时保持原镜头。
...
- 未分区实例保持显示。
- 0 匹配不回退。
- 完成显隐切换后，对最终业务成员集合（含显式 Zone 交集）计算联合包围盒，并执行一次性平滑取景。跨分区业务允许形成较远的全局取景；0 匹配时保持原镜头。
...
这里的“聚焦”是一次性平滑取景：相机按实例包围盒计算观察中心和距离，移动到可完整观察实例的位置。它不是持续跟踪移动实例，也不会把漫游人物传送到实例旁；漫游状态会切换到可自由移动的观察相机，按 `V` 可回到人物视角。
