...
| 当前会话人物 | `active_character_id` | 从 UE Dock 临时选择的本地观察者；只在当前漫游会话生效，不写入 ProjectStore |
...
| 选中 | 近身/第一人称准星 + `E`，或释放右键后的鼠标左键；上帝视角释放右键后的鼠标左键；共用选择服务 |
...
- Shift 奔跑，C 下蹲，Space 在近身与第一人称模式下始终用于跳跃。P 在自动路线/安全归线中切换暂停与继续；点位解说期间只切换“解说后暂停/继续”，字幕和语音保持播放。
...
- 近身/第一人称采用两态鼠标：释放右键时显示并释放指针，左键按指针位置执行场景射线；按住右键时切换为 `GameOnly`，隐藏并捕获鼠标，仅用于镜头观察。
- 近身/第一人称的 `E` 始终使用屏幕中心准星，不依赖鼠标指针。
- 上帝视角沿用同一右键捕获规则：按住右键观察，释放后左键按指针位置选择。
- 右键按住期间左键不触发对象选择，避免观察动作与点击动作冲突。
- `Tab` 切换 HUD 交互模式；进入后强制显示鼠标并暂停人物输入和相机输入，关闭后恢复指针交互态。
...
| 近身 | 鼠标右键（按住） | 环绕/俯仰观察；释放后鼠标可点击 |
| 近身 | 鼠标左键（释放右键后） | 按指针位置选中对象/打开信息面板 |
| 近身 | `Shift` | 奔跑 |
| 近身 | `Space` | 跳跃；不再承担路线恢复或暂停语义 |
| 近身 | `P` | 自动路线/安全归线中暂停或继续；点位解说期间切换解说后的暂停状态且不停止语音 |
| 近身 | `C` | 下蹲 |
| 近身 | `E` | 准星选中对象/打开信息面板 |
| 近身 | `R` | 尝试继续默认路线 |
| 上帝 | `WASD` | 平面移动自由相机 |
| 上帝 | `Q/E` | 相机下降/上升 |
| 上帝 | 鼠标右键（按住） | 观察方向；释放后鼠标可点击 |
| 上帝 | 滚轮 | 调整相机移动速度或等价缩放 |
| 上帝 | `P` | 自动路线/安全归线中暂停或继续；Space 不执行动作 |
| 上帝 | 鼠标左键（释放右键后） | 按指针位置选中对象/打开信息面板 |
...
- 实际生效快捷键帮助。
- 当前宿主已安装人物的会话选择；切换后原地生效，不重启、不跳转 Web。
- 允许皮肤选择。
- 继续路线 / 从头开始。
- 结构配置待重载时的“重新加载人物”。
- 降级项和可执行的处理建议。
...
| 会话态 | Dock 人物选择 | 原地替换 Mesh、动画和兼容皮肤；保留脚底位置、朝向、视角、路线进度及正在播放的解说；退出漫游后丢弃 |
| 生命周期 | `enabled` | false 时安全退出；true 时进入 available |
| 下次进入 | `auto_enter`、默认视角 | 不在当前会话中强制抢占；下次进入生效 |
| 待重载 | Web 项目默认 `character_id` | 需要重新生成人物；与 Dock 的当前会话人物互不改写 |
...
Dock 只展示资源目录中且当前宿主实际安装成功的人物。切换失败必须回滚到原人物；不得停止独立的解说音频组件，也不得重置 RouteFollower 会话。
