...
- 路线修改与项目默认声音修改分别保存、互不禁用；保存任一侧不得丢弃或覆盖另一侧尚未保存的草稿。
...
- 有字幕时最大宽度约视口 `42%`，正文最多三行；更长内容已由后端分页。纯语音且无字幕时收缩为紧凑胶囊，不保留空白长条。
- 显示当前页文本和纯数字 `当前段/总段` 进度；不显示“路线解说”“字幕”“语音”等常驻标签。
- “跳过当前段”改为 `28×28` logical px 的双箭头图标按钮，悬停提示和可访问名称仍为“跳过当前段”。
...
- Performance：静态可读表面。
- 使用独立场景采样/模糊层、主题着色与边框层、细噪点/顶部高光层、锐利内容层；禁止退化为单层深色透明黑卡。
...
| P | 自动移动时暂停/继续路线；解说播放时只切换“解说后暂停/继续”，当前字幕与语音不停 |
| Space | 近身与第一人称模式下跳跃，不改变路线暂停状态 |
| F7 退出 | 立即停止并销毁 Narration 状态，恢复宿主输入和声音 |
| V 切换视角 | 解说继续，不重新触发、不打断音频 |
| Dock 切换人物 | 原地替换人物表现，解说继续，不重置当前段和路线进度 |
...
- 路线与项目默认声音同时存在未保存修改时，两个保存入口均可用，并可按任意顺序安全保存。
